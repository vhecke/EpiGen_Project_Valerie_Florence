...
For annotating genes we chose to exclude all peaks with values that are 0 to have heatmaps that are a bit more easy to read.
```{r}
...
```
For the scope of this project, screening up to 150 different genes would be too extensive. Therefore from each heatmap, 4 clusters, one from every hPMT, were chosen visually and the genes in this cluster were screened for function in the cell using enrichR. 
