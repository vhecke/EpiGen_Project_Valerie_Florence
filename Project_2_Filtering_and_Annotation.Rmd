...
This PCA demonstrates that the global patterns of hPMTs distinctly separate both by mark and by T cell state. Samples cluster tightly by hPMT (shape), and within each mark, further group by cell type (color), indicating that both the choice of histone mark and the differentiation state of the T cell are major drivers of epigenetic variation. The absence of overlap between hPMT driven clusters suggests robust and reproducible differences in chromatin state among naïve, memory, and exhausted T cells. 

## Differential hPMT Analysis in Naïve vs. Memory T Cells
...
pseudocount <- 1e-6
## H3K4me3
mean_naïve_H3K4me3 <- rowMeans(H3K4me3_signal[, 1:4])   # 40269 elements
mean_memory_H3K4me3 <- rowMeans(H3K4me3_signal[, 5:8])  
mean_exh_H3K4me3 <- rowMeans(H3K4me3_signal[, 9:12])

log2_H3K4me3 <- log2((mean_memory_H3K4me3 + pseudocount) / (mean_naïve_H3K4me3 + pseudocount))
...
## H3K27ac
mean_naïve_H3K27ac <- rowMeans(H3K27ac_signal[, 1:4])  
mean_memory_H3K27ac <- rowMeans(H3K27ac_signal[, 5:8])  
mean_exh_H3K27ac <- rowMeans(H3K27ac_signal[, 9:12])  

log2_H3K27ac <- log2((mean_memory_H3K27ac + pseudocount) / (mean_naïve_H3K27ac + pseudocount))
...
## H3K27me3
mean_naïve_H3K27me3 <- rowMeans(H3K27me3_signal[, 1:4])   
mean_memory_H3K27me3 <- rowMeans(H3K27me3_signal[, 5:8]) 
mean_exh_H3K27me3 <- rowMeans(H3K27me3_signal[, 9:12])  

log2_H3K27me3 <- log2((mean_memory_H3K27me3 + pseudocount) / (mean_naïve_H3K27me3 + pseudocount))
...
## H3K9me3
mean_naïve_H3K9me3 <- rowMeans(H3K9me3_signal[, 1:4])   
mean_memory_H3K9me3 <- rowMeans(H3K9me3_signal[, 5:8]) 
mean_exh_H3K9me3 <- rowMeans(H3K9me3_signal[, 9:12])  

log2_H3K9me3 <- log2((mean_memory_H3K9me3 + pseudocount) / (mean_naïve_H3K9me3 + pseudocount))
...
### Naïve and exhausted T-cells:
```{r}
## H3K4me3
log2_H3K4me3_EXN <- log2((mean_exh_H3K4me3 + pseudocount) / (mean_naïve_H3K4me3 + pseudocount))
...
## H3K27ac
log2_H3K27ac_EXN <- log2((mean_exh_H3K27ac + pseudocount) / (mean_naïve_H3K27ac + pseudocount))
...
# H3K27me3
log2_H3K27me3_EXN <- log2((mean_exh_H3K27me3 + pseudocount) / (mean_naïve_H3K27me3 + pseudocount))
...
# H3K9me3
log2_H3K9me3_EXN <- log2((mean_exh_H3K9me3 + pseudocount) / (mean_naïve_H3K9me3 + pseudocount))
...
## Visualisation of Cell Type Comparison

### Naïve vs memory T-cell
...
anno_MN_filtered  <- anno_MN[apply(anno_MN != 0, 1, all), , drop = FALSE]
dim(anno_MN_filtered)
anno_EXN_filtered <- anno_EXN[apply(anno_EXN != 0, 1, all), , drop = FALSE]
dim(anno_EXN_filtered)
anno_EXM_filtered <- anno_EXM[apply(anno_EXM != 0, 1, all), , drop = FALSE]
dim(anno_EXM_filtered)
```
The first three matrices all contain 5000 peaks, sicne we previously filtered for the top 5000 peaks in every hPMT. Post filtering out any rows that contained a zero, i.e. not significant across all hPMTs, the datasets were substantially smaller.

```{r}
annot_hmap1 <- pheatmap::pheatmap(anno_MN_filtered, breaks = my_breaks, 
         annotation_col = annotation_col,
         annotation_colors = annotation_colors,
         show_colnames = FALSE,
         show_rownames = FALSE,
...
annot_hmap2 <- pheatmap::pheatmap(anno_EXN_filtered, breaks = my_breaks, 
         annotation_col = annotation_col_2,
         annotation_colors = annotation_colors_2,
         show_colnames = FALSE,
         show_rownames = FALSE,
...
annot_hmap3 <- pheatmap::pheatmap(anno_EXM_filtered, breaks = my_breaks, 
         annotation_col = annotation_col_3,
         annotation_colors = annotation_colors_3,
         show_colnames = FALSE,
         show_rownames = FALSE,
...
grid.arrange(
  ...
  annot_hmap3$gtable, 
  ncol = 1,
  top = "Heatmaps of Differential Histone Modification Signals with Zero-Value Peaks Removed"
)
```
...
## Numerical evaluation of activating, repressing and bivalent regions across cell type comparisons
